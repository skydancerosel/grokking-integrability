...
The lead time grows super-linearly with grokking timescale ($\alpha \approx 1.3$): at $\eta = 10^{-4}$, defect onset occurs within the first 7\% of training, providing a 93\% advance warning window.
...
The predictive lead time (\Cref{fig:lr_phase}D) is largest at $\eta = 10^{-4}$ ($\sim$26--38k steps, 93\% of training time) and moderate at $\eta = 10^{-3}$ ($\sim$800--2200 steps, 46\% of training time).
Crucially, the lead time grows \emph{super-linearly} with grokking timescale: a 10$\times$ increase in grok step (from $\sim$3k to $\sim$34k) produces a 23$\times$ increase in lead time, yielding a power-law exponent $\alpha \approx 1.3$.
...
Notably, the defect onset lead time grows \emph{super-linearly} with grokking timescale (power-law exponent $\alpha \approx 1.3$), meaning the predictive window improves at slower, more realistic learning rates---at $\eta = 10^{-4}$, defect onset occurs at only $\sim$7\% of training time, providing a 93\% advance warning window.
